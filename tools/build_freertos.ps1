...
foreach ($FileName in @("main.c", "app_cfg.h")) {
    ...
    Copy-Item `
        (Join-Path $SourceAppFolder $FileName) `
        (Join-Path $SdkAppFolder $FileName) `
        -Force
}
...
try {
    if ($Clean) {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "MSDK clean failed with code $LASTEXITCODE."
        }
    }

    & cmd.exe /c "cmake_build.bat app -j$Jobs"
    if ($LASTEXITCODE -ne 0) {
        throw "MSDK build failed with code $LASTEXITCODE."
    }
}
finally {
    Pop-Location
